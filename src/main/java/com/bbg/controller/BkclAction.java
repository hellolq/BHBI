...
public class BkclAction {
    ...
    private final static Logger logger = (Logger) LoggerFactory.getLogger(BkclAction.class);
    ...
    public ResponseObj bkcl(BfclDTO source){
        ...
        try {
            result = bkclService.parsonDsJson(source.getOrderJson());
            ...
            logger.info("电商传单-异常:{}",e.getMessage());
        }finally {
            logger.info("电商传单-OrderJson:{}",source.getOrderJson());
            logger.info("电商传单-result:{}",result);
        }
